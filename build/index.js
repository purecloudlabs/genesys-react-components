...
function DxAccordion(props) {
	...
	let icon;
	if (props.headingIcon) icon = React.createElement(GenesysDevIcon, { icon: props.headingIcon, className: 'heading-icon' });
	return React.createElement(
		'div',
		{ id: props.containerId || undefined, className: `dx-accordion${props.className ? ' ' + props.className : ''}` },
		React.createElement(
			'div',
			{ className: 'accordion-heading', style: style, onClick: () => setIsOpen(!isOpen) },
			React.createElement('span', { className: 'accordion-heading__left' }, icon, ' ', props.title),
			' ',
...
function DxButton(props) {
	...
	return React.createElement(
		'button',
		{ className: classNames.join(' '), type: 'button', onClick: handleClick, disabled: props.disabled === true },
		props.children
	);
}
...
function DxCheckbox(props) {
	...
	return React.createElement(
		'label',
		{ className: `dx-checkbox${props.className ? ' ' + props.className : ''}${props.disabled ? ' disabled' : ''}` },
...
function DxItemGroup(props) {
	...
	switch (format) {
		...
		case 'dropdown': {
			...
			return React.createElement(
				DxLabel,
				{ label: title, description: description, className: className },
				React.createElement(
					'div',
					{ className: `dx-item-group${isMulti ? ' dx-multiselect-group' : ' dx-select-group'}${disabled ? ' disabled' : ''}` },
					React.createElement(
						'select',
						{
							multiple: isMulti,
							...
							value: isMulti
								? (_a = data.filter((item) => item.isSelected)) === null || _a === void 0
									? void 0
									: _a.map((item) => item.item.value)
								: (_b = data.find((item) => item.isSelected)) === null || _b === void 0
								? void 0
								: _b.item.value,
						},
						data.map((d, i) => React.createElement('option', { key: i, value: d.item.value, disabled: d.item.disabled }, d.item.label))
					)
				)
			);
		}
		...
		default: {
			return React.createElement(
				...
				React.createElement(
					'div',
					{
						onChange: (e) => {
							...
							const i = data.findIndex((d) => {
								var _a;
								return d.item.value === ((_a = e.target) === null || _a === void 0 ? void 0 : _a.value);
							});
							if (i < 0) return;
					...
					data.map((d, i) =>
						React.createElement(DxCheckbox, {
							key: d.item.value,
...
function DxTabbedContent(props) {
	...
	return React.createElement(
		'div',
		{ className: `dx-tabbed-content${props.className ? ' ' + props.className : ''}` },
		React.createElement(
			'div',
			...
			titles.map((title, i) =>
				React.createElement(
					'span',
					{ key: i, className: `tab-title${i === activeTab ? ' active' : ''}`, onClick: () => setActiveTab(i) },
					title
				)
			)
		),
...
function DxTextbox(props) {
	...
	else if (props.inputType === 'textarea') inputRef = useRef(null);
	...
	switch (inputType) {
		...
		default: {
			component = React.createElement(
				'div',
				{ className: `dx-textbox${hasLabel ? ' with-label' : ''}${props.disabled ? ' disabled' : ''}` },
				...
				props.clearButton && (value || isFocused) && !props.disabled
					? React.createElement(GenesysDevIcon, { icon: GenesysDevIcons.AppTimes, className: 'clear-icon', onClick: () => setValue('') })
					: undefined
			);
		}
	}
...
function DxToggle(props) {
	...
	return React.createElement(
		DxLabel,
		{ label: props.label, description: props.description, className: props.className },
		React.createElement(
			'div',
			{ className: `dx-toggle-container${props.disabled ? ' disabled' : ''}` },
			React.createElement(
				'div',
				{ className: 'dx-toggle', onClick: toggleValue },
				...
				React.createElement(
					'div',
					{ className: 'slider' },
					value !== undefined ? React.createElement(GenesysDevIcon, { icon: value ? trueIcon : falseIcon }) : undefined
				),
...
function AlertBlock(props) {
	...
	return React.createElement(
		...
		React.createElement(
			'div',
			{ className: `alert alert-${props.alertType}`, role: 'alert' },
			icon,
			React.createElement(
				'div',
			...
			isCollapsible
				? React.createElement(
						'span',
						{ className: 'clickable', onClick: () => setIsCollapsed(!isCollapsed) },
						React.createElement(GenesysDevIcon, { icon: isCollapsed ? GenesysDevIcons.AppChevronDown : GenesysDevIcons.AppChevronUp })
				  )
				: undefined
		)
	);
...
function Tooltip(props) {
	...
	const showTip = (e) => {
		...
		if (e.target.className.includes('tooltip-tip')) return;
	...
	return React.createElement(
		'div',
		{ className: `tooltip-container ${props.className || ''}`, onMouseEnter: showTip, onMouseLeave: hideTip },
		props.children,
...
function CopyButton(props) {
	...
	return React.createElement(
		...
		React.createElement(
			Tooltip,
			{ isShowing: copyState, text: 'Copied', position: props.tooltipPosition },
			React.createElement(
				'button',
				{ type: 'button', className: buttonClasses.join(' '), onClick: copyCode, onMouseOut: loseFocus },
...
(function (module, exports) {
	(function (global, factory) {
		...
		function isFunction(input) {
			return (
				(typeof Function !== 'undefined' && input instanceof Function) || Object.prototype.toString.call(input) === '[object Function]'
			);
		}
...
function DataTable(props) {
				...
				.forEach((colId) => {
					...
					switch (filter.dataType) {
						...
						case 'number': {
							...
							if (filter.filterModifier === 'greaterthan' && row.cells[colId].parsedContent >= filter.filter) filterMatch = true;
							else if (filter.filterModifier === 'lessthan' && row.cells[colId].parsedContent <= filter.filter) filterMatch = true;
	...
	const sortRows = () => {
		...
		return [...filteredRows].sort((a, b) => {
			if (a.cells[i].parsedContent < b.cells[i].parsedContent) return isAscending ? -1 : 1;
			if (a.cells[i].parsedContent > b.cells[i].parsedContent) return isAscending ? 1 : -1;
			return 0;
		});
	};
	...
	const isSortable =
		props.sortable ||
		((_a = props.className) === null || _a === void 0 ? void 0 : _a.includes('sortable')) ||
		((_b = props.className) === null || _b === void 0 ? void 0 : _b.includes('sort-and-filter'));
	const isFilterable =
		props.filterable ||
		((_c = props.className) === null || _c === void 0 ? void 0 : _c.includes('filterable')) ||
		((_d = props.className) === null || _d === void 0 ? void 0 : _d.includes('sort-and-filter'));
	...
	let columnHeaders;
	if (props.headerRow) {
		columnHeaders = React.createElement(
			'tr',
			null,
			props.headerRow.cells.map((cell, i) =>
				React.createElement(
					...
					{
						key: i,
						align: (cell === null || cell === void 0 ? void 0 : cell.align) || 'left',
						className: colsort.colId === i && colsort.sort !== 'none' ? '' : 'unsorted',
						onClick: isSortable ? () => sortChanged(i.toString()) : undefined,
					},
					React.createElement(
						'div',
						{ className: `header-container align-${(cell === null || cell === void 0 ? void 0 : cell.align) || 'left'}` },
						cell.renderedContent || cell.content,
						filters[i] && filters[i].filter !== '' && filters[i].filter !== undefined
							? React.createElement(GenesysDevIcon, { icon: GenesysDevIcons.AppFilter, className: 'filter-active-icon' })
							: '',
						...
						case 'date': {
							return React.createElement(
								...
								React.createElement(
									'div',
									{ className: 'sort-date' },
									React.createElement(DxTextbox, {
										className: 'date-filter',
										label: 'Filter date',
										inputType: 'date',
										onChange: (value) => dateFilterChanged(colId, value),
										initialValue: moment.isMoment((_a = filters[i]) === null || _a === void 0 ? void 0 : _a.filter)
											? (_b = filters[i]) === null || _b === void 0
												? void 0
												: _b.filter.format('YYYY-MM-DD')
											: undefined,
									}),
									React.createElement(DxToggle, {
										label: 'Comparison',
										falseIcon: GenesysDevIcons.AppChevronLeft,
										trueIcon: GenesysDevIcons.AppChevronRight,
										initialValue: ((_c = filters[i]) === null || _c === void 0 ? void 0 : _c.filterModifier) === 'greaterthan',
							...
							return React.createElement(
								...
								React.createElement(
									...
									React.createElement(DxTextbox, {
										...
										placeholder:
											(_e = (_d = props.headerRow) === null || _d === void 0 ? void 0 : _d.cells[i]) === null || _e === void 0
												? void 0
												: _e.content,
										initialValue: (_f = filters[i]) === null || _f === void 0 ? void 0 : _f.filter,
									}),
									React.createElement(DxToggle, {
										label: 'Comparison',
										falseIcon: GenesysDevIcons.AppChevronLeft,
										trueIcon: GenesysDevIcons.AppChevronRight,
										initialValue: ((_g = filters[i]) === null || _g === void 0 ? void 0 : _g.filterModifier) === 'greaterthan',
						...
						default: {
							return React.createElement(
								...
								React.createElement(DxTextbox, {
									...
									placeholder:
										(_j = (_h = props.headerRow) === null || _h === void 0 ? void 0 : _h.cells[i]) === null || _j === void 0
											? void 0
											: _j.content,
									onChange: (value) => stringFilterChanged(colId, value),
									clearButton: true,
									initialValue: (_k = filters[i]) === null || _k === void 0 ? void 0 : _k.filter,
								})
							);
						}
					}
	...
	if (columnHeaders || filterRow) {
		thead = React.createElement('thead', null, columnHeaders, isFilterOpen ? filterRow : undefined);
	}
	...
	return React.createElement(
		'div',
		{ className: `table-container${isSortable ? ' sortable' : ''}${isFilterable ? ' filterable' : ''}` },
		React.createElement(
			'div',
			{ className: 'filter-container' },
			React.createElement(
				'div',
				{ className: 'filter-toggle', style: { visibility: isFilterable ? 'visible' : 'hidden' } },
				React.createElement(GenesysDevIcon, { icon: GenesysDevIcons.AppFilter, onClick: () => setIsFilterOpen(!isFilterOpen) })
			),
			React.createElement(
				'table',
				{ className: tableClassName, cellSpacing: '0' },
				thead,
				React.createElement(
					'tbody',
					null,
					rows.map((row, i) =>
						React.createElement(
							...
							row.cells.map((cell, ii) =>
								React.createElement(
									'td',
									{ key: ii, align: (cell === null || cell === void 0 ? void 0 : cell.align) || 'left' },
									(cell === null || cell === void 0 ? void 0 : cell.content)
										? React.createElement(
												'div',
												{ className: `align-${(cell === null || cell === void 0 ? void 0 : cell.align) || 'left'}` },
												cell.renderedContent || cell.content,
												cell.copyButton ? React.createElement(CopyButton, { copyText: cell.content }) : undefined
										  )
										: null
								)
							)
	...
	return React.createElement(
		...
		props.noHeader || typeof props.value !== 'string'
			...
			: React.createElement(
					...
					props.noCollapse
						? undefined
						: React.createElement(GenesysDevIcon, { icon: collapsed ? GenesysDevIcons.AppChevronDown : GenesysDevIcons.AppChevronUp }),
		...
		collapsed
			...
			: React.createElement(
					'div',
					{ ref: props.innerRef || undefined, className: bodyClassNames.join(' ') },
					React.createElement(
						PrismAsync,
						{ language: props.language, style: vscDarkPlus, showLineNumbers: props.showLineNumbers },
						props.value
					)
			  )
	);
